...
| --- | --- | --- | --- |
| `dotnet --version` | PASS: `10.0.301` selected through `global.json` | The repository uses the checked-in stable .NET 10 SDK policy. | Availability of that SDK on another host. |
| `dotnet build ChangeRiskAgent.slnx -c Release --no-restore` | PASS: both `net10.0` projects; 0 warnings, 0 errors | The application and offline test executable compile in Release on .NET 10. | Authenticated Foundry service availability. |
| `dotnet run --project src/ChangeRiskAgent -c Release --no-build -- CHG-1001` | PASS; low-risk advisory emitted | The documented happy path is runnable from the repository root. | Model quality, latency, cost, quota, or capacity. |
...
| `dotnet format ChangeRiskAgent.slnx --verify-no-changes --no-restore` | PASS | All tracked C# sources conform to the configured SDK formatter. | Runtime behavior. |
...
| The local machine only had the stable .NET 10 SDK. | A requested .NET 8 scaffold was not available. | Standardized both projects on `net10.0` and pinned SDK `10.0.301` with stable patch roll-forward. | Installed SDK/template availability differed from the initial assumption. | Discover and pin the intended SDK before scaffolding or validation. |
